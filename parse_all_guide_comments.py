...
def get_guide_name(url: str) -> str:
    r = requests.get(url)
    soup = bs(r.text, "lxml")
    return soup.find(id="firstHeading", class_="firstHeading").text.strip()


def get_all_guides_urls() -> typing.List[str]:
    url = "http://wikimipt.org/wiki/%D0%9A%D0%B0%D1%82%D0%B5%D0%B3%D0%BE%D1%80%D0%B8%D1%8F:%D0%9F%D1%83%D1%82%D0%B5%D0%B2%D0%BE%D0%B4%D0%B8%D1%82%D0%B5%D0%BB%D1%8C"
    r = requests.get(url)
    soup = bs(r.text, "lxml")

    urls: typing.List[str] = []

    raw_groups_data = soup.find(id="mw-pages").find(class_="mw-content-ltr").find_all("ul")

    for raw_group_data in raw_groups_data:
        raw_urls_data = raw_group_data.find_all("li")
...
def get_all_comments_by_page(page_url: str) -> typing.List[comment.GuideComment]:
    comments: typing.List[comment.GuideComment] = []
    comment_type = comment.CommentType.GUIDE

    print(f"Parsing guide: {get_guide_name(page_url)}")
    ...
    for raw_comment_data in raw_comments_data:
        ...
        comm = comment.GuideComment(type=comment_type, author=author, date=date, time=time, score=score, text=text, hidden=hidden, reply=reply)
        comm.guide_name = get_guide_name(page_url)
...
def parse_all_guide_comments():
    print(f"Parsing guide comments...")
    comments: typing.List[comment.GuideComment] = []

    for guide_url in get_all_guides_urls():
        for comm in get_all_comments_by_page(guide_url):
            comments.append(comm)
