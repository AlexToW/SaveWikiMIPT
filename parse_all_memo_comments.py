...
def get_memo_name(url: str) -> str:
    r = requests.get(url)
    soup = bs(r.text, "lxml")
    return soup.find(id="firstHeading", class_="firstHeading").text.strip()


def get_all_memos_urls() -> typing.List[str]:
    url = "http://wikimipt.org/wiki/%D0%9A%D0%B0%D1%82%D0%B5%D0%B3%D0%BE%D1%80%D0%B8%D1%8F:%D0%9F%D0%B0%D0%BC%D1%8F%D1%82%D0%BA%D0%B8_%D0%B8_%D0%B8%D0%BD%D1%81%D1%82%D1%80%D1%83%D0%BA%D1%86%D0%B8%D0%B8"
    r = requests.get(url)
    soup = bs(r.text, "lxml")

    urls: typing.List[str] = []

    raw_groups_data = soup.find(class_="mw-content-ltr").find(id="mw-pages").find_all(class_="mw-content-ltr")
...
def get_all_comments_by_page(page_url: str) -> typing.List[comment.MemoComment]:
    comments: typing.List[comment.MemoComment] = []
    comment_type = comment.CommentType.MEMO

    print(f"Parsing memo: {get_memo_name(page_url)}")
    ...
    for raw_comment_data in raw_comments_data:
        ...
        comm = comment.MemoComment(type=comment_type, author=author, date=date, time=time, score=score, text=text, hidden=hidden, reply=reply)
        comm.memo_name = get_memo_name(page_url)
...
def parse_all_memo_comments():
    print(f"Parsing memo comments...")
    comments: typing.List[comment.MemoComment] = []

    for memo_url in get_all_memos_urls():
        for comm in get_all_comments_by_page(memo_url):
            comments.append(comm)
