...
def get_subject_name(url: str) -> str:
    r = requests.get(url)
    soup = bs(r.text, "lxml")
    return soup.find(id="firstHeading", class_="firstHeading").text.strip()


def get_all_subjects_urls() -> typing.List[str]:
    url = "http://wikimipt.org/wiki/%D0%9A%D0%B0%D1%82%D0%B5%D0%B3%D0%BE%D1%80%D0%B8%D1%8F:%D0%9F%D1%80%D0%B5%D0%B4%D0%BC%D0%B5%D1%82%D1%8B_%D0%BF%D0%BE_%D0%B0%D0%BB%D1%84%D0%B0%D0%B2%D0%B8%D1%82%D1%83"
    r = requests.get(url)
    soup = bs(r.text, "lxml")
...
def get_all_comments_by_page(page_url: str) -> typing.List[comment.SubjectComment]:
    comments: typing.List[comment.SubjectComment] = []
    comment_type = comment.CommentType.SUBJECT

    print(f"Parsing subject: {get_subject_name(page_url)}")
    ...
    for raw_comment_data in raw_comments_data:
        ...
        comm = comment.SubjectComment(type=comment_type, author=author, date=date, time=time, score=score, text=text, hidden=hidden, reply=reply)
        comm.subject_name = get_subject_name(page_url)
...
def parse_all_subject_comments():
    print(f"Parsing subject comments...")
    comments: typing.List[comment.SubjectComment] = []

    for subject_url in get_all_subjects_urls():
        for comm in get_all_comments_by_page(subject_url):
            comments.append(comm)
