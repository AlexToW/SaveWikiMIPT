...
def get_organization_name(url: str) -> str:
    r = requests.get(url)
    soup = bs(r.text, "lxml")
    return soup.find(id="firstHeading", class_="firstHeading").text.strip()


def get_all_organizations_urls() -> typing.List[str]:
    url = "http://wikimipt.org/wiki/%D0%9A%D0%B0%D1%82%D0%B5%D0%B3%D0%BE%D1%80%D0%B8%D1%8F:%D0%9E%D1%80%D0%B3%D0%B0%D0%BD%D0%B8%D0%B7%D0%B0%D1%86%D0%B8%D0%B8"
    r = requests.get(url)
    soup = bs(r.text, "lxml")
...
def get_all_comments_by_page(page_url: str) -> typing.List[comment.OrganizationComment]:
    comments: typing.List[comment.OrganizationComment] = []
    comment_type = comment.CommentType.ORGANIZATION

    print(f"Parsing organization: {get_organization_name(page_url)}")
    ...
    for raw_comment_data in raw_comments_data:
        ...
        comm = comment.OrganizationComment(type=comment_type, author=author, date=date, time=time, score=score, text=text, hidden=hidden, reply=reply)
        comm.organization_name = get_organization_name(page_url)
...
def parse_all_organization_comments():
    print(f"Parsing organization comments...")
    comments: typing.List[comment.OrganizationComment] = []

    for organization_url in get_all_organizations_urls():
        for comm in get_all_comments_by_page(organization_url):
            comments.append(comm)
