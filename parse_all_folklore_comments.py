...
def get_folklore_name(url: str) -> str:
    r = requests.get(url)
    soup = bs(r.text, "lxml")
    return soup.find(id="firstHeading", class_="firstHeading").text.strip()


def get_all_folklores_urls() -> typing.List[str]:
    url = "http://wikimipt.org/wiki/%D0%9A%D0%B0%D1%82%D0%B5%D0%B3%D0%BE%D1%80%D0%B8%D1%8F:%D0%A4%D0%B8%D0%B7%D1%82%D0%B5%D1%85%D0%BE%D0%B2%D1%81%D0%BA%D0%B8%D0%B9_%D1%84%D0%BE%D0%BB%D1%8C%D0%BA%D0%BB%D0%BE%D1%80"
    r = requests.get(url)
    soup = bs(r.text, "lxml")
...
def get_all_comments_by_page(page_url: str) -> typing.List[comment.FolkloreComment]:
    comments: typing.List[comment.FolkloreComment] = []
    comment_type = comment.CommentType.FOLKLORE

    print(f"Parsing folklore: {get_folklore_name(page_url)}")
    ...
    for raw_comment_data in raw_comments_data:
        ...
        comm = comment.FolkloreComment(type=comment_type, author=author, date=date, time=time, score=score, text=text, hidden=hidden, reply=reply)
        comm.folklore_name = get_folklore_name(page_url)
...
def parse_all_folklore_comments():
    print(f"Parsing folklore comments...")
    comments: typing.List[comment.FolkloreComment] = []

    for folklore_url in get_all_folklores_urls():
        for comm in get_all_comments_by_page(folklore_url):
            comments.append(comm)
